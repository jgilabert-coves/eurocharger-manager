...
import type { TransactionsDataTableItem, TransactionsDataTableResponse } from 'src/types/transactions';
...
const columns: GridColDef<TransactionsDataTableItem>[] = [
    ...
    { field: 'date', headerName: 'Date', flex: 1, editable: false,
        renderCell: (params) => (
            <span style={{ color: 'blue' }}>{fDateTime(params.value)}</span>
        ) 
    },
    { field: 'power', headerName: 'Power', flex: 0, editable: false,
        renderCell: (params) => (
            <span>{(params.value ? round(params.value, 2) : 0.0)} kWh</span>
        )
    },
    { field: 'total', headerName: 'Total', flex: 0, editable: false,
        renderCell: (params) => (
            <span>{(params.value ? round(params.value, 2) : 0)} €</span>
        )
    },
...
export default function TransactionsFinishedView() {
    const fetchTransactions = async (page: number, pageSize: number, searchQuery: string, sortQuery: GridSortModel) => {
        try {
            ...
            const result: TransactionsDataTableResponse = await fetcher([endpoints.transactions.current, queryArgs]);
    ...
    return (
        <>
            ...
            <DashboardContent>
                ...
                <DataTable
                    columns={columns}
                    fetchData={(page: number, pageSize: number, searchQuery: string, sortQuery?: GridSortModel) => fetchTransactions(page, pageSize, searchQuery, sortQuery || [])}
                    initialPageSize={10}
