...
const MANUAL_STEPS_CONNECTOR = ['Método', 'Información básica', 'Tramos', 'Resumen'];
...
// ----------------------------------------------------------------------
// Step 3 (Manual, non-Hubject, non-connector) – Station assignment
// ----------------------------------------------------------------------
...
function ExcelFileStep({
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setFile(f);
    if (f && assignmentMethod) onRequestPreview(f, assignmentMethod);
  };
...
export type CreateRateWizardProps = {
...
export function CreateRateWizard({ onSuccess, onCancel, connectorId, chargepointId }: CreateRateWizardProps) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitError(null);
    try {
      let newRateId: number | undefined;
      if (method === 'manual') {
        ...
        const res = await post(endpoints.rates.createFromExcel, formData);
        newRateId = res?.data?.id ?? res?.id;
      }
  ...
  return (
    <Stack spacing={3}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        <Button
          variant="contained"
          onClick={isLastStep ? handleSubmit : () => setStepIndex((s) => s + 1)}
